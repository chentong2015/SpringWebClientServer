...
/**
 * Spring MVC : Web Framework built on Servlet API
 * MVC Pattern: Model-View-Controller 划分APP成3个组成部分
 * 1. Model   : application date model, business logic and rules
 * 2. View    : display, web report, form, charts
 * 3. Controller : invoke model to perform business logic and update views output
 */
public class BaseSpringMVC {

    // Apache Maven War Project 官方文档 http://maven.apache.org/plugins/maven-war-plugin/usage.html
    /**
     * 添加 Maven War Plugin  ===>  War Plugin 要求web.xml文件必须存在(因为涉及到DispatcherServlet的配置和注册) !!
    ...
    /**
     * 添加/webapp:                           ====> Web App Resources 资源的默认路径设置：Project Structure > Facets > Web > Web resource Directories
     ...
     * 3. 可以在目录中创建新的目录/webapp/WEB-INF/views, 暴露views by dispatcher server   ===> 使用Controller来access
     */
     ...
     * -----------------
     * 1. 运行War插件action   > war:war 在/target下生成指定.war文件 spring-mvc-1.0-SNAPSHOT.war    ====> 直接使用mvn:clean install来构建 !!
